...
class GRNClassifierModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        acc = self.train_accuracy(pred_probs, y)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = F.cross_entropy(out, y)  
        pred_probs = torch.softmax(out, dim=1)
        acc = self.val_accuracy(pred_probs, y)
...
class GRNDataset:
    ...
    def extract_features(self, g: gt.Graph) -> np.ndarray:
        ...
        ev, eig = gt.eigenvector(g, weight=weights)
...
class GRNDataModule(pl.LightningDataModule):
    def __init__(self, dataset: List[Data], batch_size: int = 2,

...
from src.modules._gedvae import GEDVAE
from src.modules._splitter import SemiSupervisedDataSplitter
...
class GEDVI(RNASeqMixin, VAEMixin, ArchesMixin, BaseModelClass):
    ...
    def predict(
        ...
        for _, tensors in enumerate(scdl):
            ...
            g = tensors.get(REGISTRY_KEYS.GENE_EMB_KEY) if not ignore_embedding else None
            ...
            # Predict based on X
            pred = pred_out[MODULE_KEYS.X_KEY]
    ...
    def get_latent_representation(
        ...
        ----------
        adata : AnnData | None
            AnnData object with var_names in the same order as the training data.
            Defaults to the object used to initialize the model if None and dataloader is also None.
        indices : Sequence[int] | None
            Indices of observations in adata to use. Ignored if dataloader is provided.
        give_mean : bool
            If True, returns the mean of the latent distribution. Otherwise, uses Monte Carlo sampling.
        mc_samples : int
            Number of Monte Carlo samples for estimating the mean. Ignored if give_mean is True or return_dist is True.
        batch_size : int | None
            Minibatch size for the forward pass. Ignored if dataloader is provided.
        return_dist : bool
            If True, returns the mean and variance of the latent distribution.
        ignore_embedding : bool
            If True, ignores gene embeddings in the computation.
        dataloader : Iterator[dict[str, torch.Tensor | None]] | None
            Iterator over minibatches of data. If None, a dataloader is created from adata.

        Returns
        -------
        np.ndarray or dict[str, np.ndarray]
            Latent representation or distribution statistics.
        """
        ...
        for tensors in dataloader:
            ...
            # Handle latent variables z
            qz = outputs.get(MODULE_KEYS.QZ_KEY) or Normal(
            ...
            if not ignore_embedding:
                ...
                qzgm, qzgv = qzg.loc, qzg.scale.square()

            if return_dist:
                results['z_means'].append(qzm.cpu())
                results['z_vars'].append(qzv.cpu())
                if not ignore_embedding:
                    results['zg_means'].append(qzgm.cpu())
                    results['zg_vars'].append(qzgv.cpu())
                continue
        ...
        if return_dist:
            ...
            return {
                ...
                'zg': (
                    torch.cat(results['zg_means']).numpy(),
                    torch.cat(results['zg_vars']).numpy(),
                ),
            }
        ...
        return {
            'z': torch.cat(results['zs']).numpy(),
            'zg': torch.cat(results['zgs']).numpy(),
        }
    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        if gene_emb_obsm_key is not None:

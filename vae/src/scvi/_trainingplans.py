...
class DisentangledSemiSupervisedTrainingPlan(TrainingPlan):
    def __init__(
        self,
        module: BaseModuleClass,
        n_classes_per_group: list[int],
        *,
        classification_ratio: int = 50,
        lr: float = 1e-3,
        weight_decay: float = 1e-6,
        ...
        **loss_kwargs,
        ...
        self.loss_kwargs.update({"classification_ratio": classification_ratio})
        self.n_classes_per_group = n_classes_per_group
    ...
    def compute_and_log_metrics(
        self, loss_output: LossOutput, metrics: dict[str, ElboMetric], mode: str, i: int
    ):
        """Computes and logs metrics."""
        if i==0:
            super().compute_and_log_metrics(loss_output, metrics, mode)
        ...
        logits = loss_output.logits
        predicted_labels = torch.argmax(logits, dim=-1)
        n_classes = self.n_classes_per_group[i]
        ...
        self.log_with_mode(
            f'{METRIC_KEYS.CLASSIFICATION_LOSS_KEY}_group_{i}',
            classification_loss,
            mode,
            on_step=False,
            on_epoch=True,
        ...
        self.log_with_mode(
            f'{METRIC_KEYS.ACCURACY_KEY}_group_{i}',
            accuracy,
            mode,
            on_step=False,
            on_epoch=True,
        ...
        self.log_with_mode(
            f'{METRIC_KEYS.F1_SCORE_KEY}_group_{i}',
            f1,
            mode,
            on_step=False,
            on_epoch=True,
        ...
        self.log_with_mode(
            f'{METRIC_KEYS.CALIBRATION_ERROR_KEY}_group_{i}',
            ce,
            mode,
            on_step=False,
            on_epoch=True,
    ...
    def training_step(self, batch, batch_idx):
        ...
        input_kwargs.update(self.loss_kwargs)
        _, _, loss_outputs = self.forward(full_dataset, loss_kwargs=input_kwargs)
        overall_loss_output = loss_outputs[0]
        loss = overall_loss_output.loss
        self.log(
            "train_loss",
            loss,
            on_epoch=True,
            batch_size=overall_loss_output.n_obs_minibatch,
            prog_bar=True,
        )
        classification_loss = 0
        accs = []
        for i, group_loss in enumerate(loss_outputs[1]):
            acc = self.compute_and_log_metrics(group_loss, self.train_metrics, "train", i)
            classification_loss += group_loss.classification_loss
            accs.append(acc)
        # weigh accuracies ranking up from start to finish
        ng = len(loss_outputs[1])
        weights = np.arange(1,ng+1) / ng
        overall_accuracy = torch.mean(torch.tensor(accs) * weights)
        self.log(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        accs = []
        for i, group_loss in enumerate(loss_outputs[1]):
            acc = self.compute_and_log_metrics(group_loss, self.val_metrics, "validation", i)
            classification_loss += group_loss.classification_loss
            accs.append(acc)
        # weigh accuracies ranking up from start to finish
        ng = len(loss_outputs[1])

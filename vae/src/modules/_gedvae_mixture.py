...
class GEDVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        emb_likelihood: Literal['zinb', 'nb', 'poisson', 'normal'] = 'normal',
        ...
        if self.use_x_in_zg:
            from scvi.nn import EncoderTOTALVI, DecoderTOTALVI
        ...
        # Setup dispersion parameters based on G encoding options
        ...
        if self.n_batch > 0:
            self.background_pro_alpha = torch.nn.Parameter(
                torch.randn(self.n_input_g, self.n_batch)
            )
            self.background_pro_log_beta = torch.nn.Parameter(
                torch.clamp(torch.randn(self.n_input_g, self.n_batch), -10, 1)
            )
        else:
            self.background_pro_alpha = torch.nn.Parameter(torch.randn(self.n_input_g))
            self.background_pro_log_beta = torch.nn.Parameter(
                torch.clamp(torch.randn(self.n_input_g), -10, 1)
            )
    ...
    def _get_generative_input(
        ...
        return {
            ...
            MODULE_KEYS.LIBRARY_G_KEY: inference_outputs[MODULE_KEYS.LIBRARY_G_KEY],
            MODULE_KEYS.BATCH_INDEX_KEY: tensors[REGISTRY_KEYS.BATCH_KEY],
    ...
    def inference(
        ...
        if g is not None:
            ...
            if self.use_x_in_zg:
                ...
                qz, ql, latent, untran_latent = self.encoder(
                    encoder_input_g, batch_index, *categorical_input
                )
            ...
            # TODO: this includes covariates, not ideal
            library_g = torch.log(torch.abs(encoder_input_g).sum(1)).unsqueeze(1)
            if self.g_weight > 0:
            ...
            qz_g, z_g, library_g, xpwd, gpwd = None, None, None, None, None
        ...
        return {
            ...
            MODULE_KEYS.LIBRARY_G_KEY: library_g,
    ...
    def _decode_z(
        ...
        decoder_input = z if cont_covs is None else torch.cat(
            [z, cont_covs.unsqueeze(0).expand(z.size(0), -1, -1)] if z.dim() != cont_covs.dim() else [z, cont_covs], dim=-1
        )
        ...
        # Decode
        scale, r, rate, dropout = self.decoder(
    ...
    def _decode_g(
        ...
        if cont_covs is None:
            decoder_input = zg
        elif zg.dim() != cont_covs.dim():
            decoder_input = torch.cat(
                [zg, cont_covs.unsqueeze(0).expand(zg.size(0), -1, -1)], dim=-1
            )
        else:
            decoder_input = torch.cat([zg, cont_covs], dim=-1)
    ...
    def generative(
        ...
        if self.decode_g:
            zg_out = self._decode(
                latent=zg, library=library_g, batch_index=batch_index, decoder=self.g_decoder,
    ...
    def loss(
        ...
        if self.decode_g and g is not None:
            ...
            # Pin loss at minimum of 0
            if torch.mean(reconst_loss_zg) < 0:
                reconst_loss_zg = torch.zeros_like(reconst_loss_zg)
        ...
        if zg is not None and contrastive_loss_weight is not None and contrastive_loss_weight > 0:
